...
**Step 8's `RouterSessionManager` half is now done (July 31, 2026)** — see `src/NetPilot.Core/RouterConnection/RouterSessionManager.cs`. Built as specified in §6.1 below: single `SemaphoreSlim(1,1)`-serialized session, `ExecuteAsync<T>(Func<IRouterProvider, Task<T>>)`, plus a `TestConnectionAsync` overload for the dashboard's "test with unsaved settings" flow that §6.1 didn't anticipate needing separately. `Worker.cs` and all of `Home.razor`'s router call sites route through it now — the two-sessions bug is closed. One correction to §6.1's own count: it said "four separate handlers" in `Home.razor`; by the time this landed there were actually eight (block/unblock and the two wireless handlers were added after §6.1 was written). The Agent-as-`WebApplication` half of step 8/step 9 is still not started.

**Decided with the user, July 31, 2026: everything else in this doc is deliberately deferred, not abandoned.** Steps 6, 7, and 9 (`NetPilot.Data` wireless store, `NetPilot.Core/Wireless` reconciliation, the Agent's wireless-specific fast-loop/sweep), the remaining live-capture items (main-network write safety, IoT password confirmation, W4 shared-radio coupling, txpower/channelWidth enumeration), and §10's two open questions (schedule ownership, drift-enforcement default) are all on hold until wireless-via-HA is actually wanted — not scheduled next. The plain "Agent becomes a `WebApplication`" conversion (step 8's other half, step 9) is **not** deferred — it still happens as part of `docs/phase5-home-assistant-integration-plan.md`'s non-wireless API work, since HA needs an HTTP surface regardless of wireless. Only the wireless-specific fast-loop/sweep addition to that conversion is on hold.
...
8. `RouterSessionManager` + Agent loop split. Migrate existing calls onto it. **`RouterSessionManager` done (July 31, 2026)** — Worker and all of Home.razor's call sites migrated, closing the "two-sessions bug." **Agent loop split (fast/slow/wireless-sweep) still not started.**

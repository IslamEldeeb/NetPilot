...
**Status (updated July 31, 2026): Implemented.** All 17 file-by-file steps below shipped, plus a daily rollup (`usage_daily_history`) not in this ticket's original scope. See `docs/phase2-usage-tracking-feature.md` for the as-built reference, including two deliberate deviations from this plan: first-ever counter observations count in full immediately (not held back as a baseline-only reading, as step 8's `UsageTrackingService` below originally specified), and a day-granularity rollup was added alongside the month one. Not yet deployed to the production CT as of the as-built doc's last edit. `trafficUsage`'s exact byte-vs-formatted-string unit (step 3's `TpLinkUsageParser` assumption) is tracked as an open item in `docs/deferred-issues.md` #1, not here.
